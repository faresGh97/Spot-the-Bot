...
def create_vertices(corpus: list, k: int) -> dict:
    ...
    text_i = [{'document_index': 1,
               'vectorized_text': {word: vector,
                                 word: vector, ...}},
              ...,
              {'document_index': m,
               'vectorized_text': {word: vector,
                                 word: vector, ...}}]
    So, we get m documents
    k : int
    Vector length
    Returns
    -------
    ...
    vertices = dict()
    for doc in tqdm(corpus, file=file_):
        for word, vector in doc['vectorized_text'].items():
...
class Graph:
    ...
    def get_networkx_graph(self) -> nx.classes.graph.Graph:
        '''
        Convert graph to NetworkX graph type
...
class Parallel(Graph):
    ...
    def parallel_sphere_radiuses (self, index:int)-> dict:
        words_sphere_radius = dict()
        file_ = open(f'stream_get_words_sphere_radius_{index}.txt', 'w')
    ...
    def parallel_influence_graph(self, index: int, spheres: dict) -> dict:
        '''
...
class GG(Graph):
    ...
    def create_delaunay_graph(self):
        ...
        file_ = open('At_delaunay_progress.txt', 'w')
    ...
    def create_parallel_gabriel_graph(self, triangles: list, num_cpus: int = 1):
        ...
        for word in self.get_words():
            updated_neighbors = self.vertices[word].neighbors.copy()
            for neighbor in updated_neighbors:
                neighbor_flag = True
                possible_edge = (word, neighbor)
                for other_word in updated_neighbors:
                    if other_word == neighbor:
                        continue
                    if self.is_neighbor_in_edge_sphere(possible_edge, other_word):
                        self.delete_edge(possible_edge[0], possible_edge[1])
                        break
        file_.close()
...
class IG(Graph):
    ...
    def create_influence_graph(self):
        ...
        '''
        self.reset_graph_neighbors()
        radiuses = self.get_full_sphere_radius()
        file_ = open('at_influnce_progress.txt', 'w')
        for first_word in tqdm(self.vertices.keys(),file=file_):
            other_words = self.get_other_words(first_word)
            #first_radius = self.get_sphere_radius(first_word)
            first_radius = radiuses[first_word]
            for second_word in other_words:
                #second_radius = self.get_sphere_radius(second_word)
                second_radius = radiuses[second_word]
                distance = self.euclid_distance(first_word, second_word)
                if distance <= first_radius + second_radius:
                    self.add_edge(first_word, second_word)
        file_.close()
    ...
    def create_parallel_influence_graph(self,num_cpus: int = 1):
        '''
        ...
        ray.init(num_cpus=num_cpus)
        streaming_actors = [Parallel.remote(self.vertices) for _ in range(num_cpus)]
        file_ = open('stream_create_radiuses_progress.txt', 'w')
        for index, word in enumerate(tqdm(self.vertices),file_):
            streaming_actors[index % num_cpus].add_parallel_word.remote(word)
        file_.write("\n Ready for ray get \n")
        results = ray.get([
            actor.parallel_sphere_radiuses.remote(index) for index ,actor in enumerate(streaming_actors)
        ])
        ray.shutdown()
        for parallel_dic in results:
            all_Radiuses.update(parallel_dic)
        ray.init(num_cpus=num_cpus)
        streaming_actors = [Parallel.remote(self.vertices) for _ in range(num_cpus)]
        for index, word in enumerate(self.vertices):
            streaming_actors[index % num_cpus].add_parallel_word.remote(word)
        results = ray.get([
            actor.parallel_influence_graph.remote(index) for index ,actor in enumerate(streaming_actors)
        ])
        ray.shutdown()
